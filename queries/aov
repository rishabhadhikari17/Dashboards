...
/*
  Determine the reporting period selected through the
...
/*
  Include seven preceding days for same-weekday comparison.
*/
calendar AS (
  SELECT
    GENERATE_SERIES(
...
/*
  Produce one row per successful order.

  This prevents one-to-many attribution and payment joins
  from duplicating revenue or orders.
*/
eligible_orders AS (
  SELECT DISTINCT
  ...
  WHERE LOWER(TRIM(ecom.orders.payment_status)) = 'paid'
    ...
    /*
      Retrieve the displayed period plus seven comparison days.
      The dashboard Date Range filter is not repeated here.
    */
...
/*
  Preserve calendar dates with no successful orders.

  AOV remains NULL for these dates because an average cannot
  be calculated when there are no orders.
*/
daily_series AS (
  SELECT
    c.metric_date,
  ...
  LEFT JOIN daily_aov AS da
    ON da.metric_date = c.metric_date
),

/*
  Compare each day's AOV with the same weekday one week earlier.
*/
daily_comparison AS (
  SELECT
...
SELECT
  ...
  ROUND(
    (
      dc.aov - dc.aov_7d_ago
    )
    / NULLIF(dc.aov_7d_ago, 0),
    4
  ) AS wow_change
